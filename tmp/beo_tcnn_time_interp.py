...
class HashMLP(pl.LightningModule):
  def __init__(self, config, dim_in=3, dim_out=1, dim_t=2):
    ...
    self.encodings = nn.ModuleList([tcnn.Encoding(n_input_dims=dim_in, encoding_config=config['encoding']) for t in range(dim_t)])
    self.mlp= tcnn.Network(n_input_dims=self.encodings[0].n_output_dims, n_output_dims=dim_out, network_config=config['network'])

  def forward(self, x):
    x_time = x[:,0].reshape(-1,1)    #time is first column
    x_pos  = x[:,1:4]  #x,y,z position are 2 to 4th colums
    t = torch.linspace(0,1,self.dim_t, device=self.device)

    #compute linear weights
    w = (self.dim_t-1)*torch.relu(1/(self.dim_t-1)-torch.abs(x_time - t))

    #weighted encoding
    w_enc = torch.zeros(x.shape[0],self.encodings[0].n_output_dims, device=self.device)
...
if __name__ == '__main__':
  ...
  parser.add_argument(      '--n_encodings', help='Number of temporal encodings (minimum 2)', type=int, required=False, default=2)
  ...
  net = HashMLP(config = config, dim_in=3, dim_out=1, dim_t=args.n_encodings)
  ...
  print(yhat.shape)
  output = np.float32(np.moveaxis(yhat.cpu().detach().numpy().reshape((nt,nx,ny,nz)),0,3))
  print(output.shape)
  ...
  pred_loader = torch.utils.data.DataLoader(pred_dataset, batch_size=batch_size, num_workers=num_workers, pin_memory=True) 
  pred = torch.concat(trainer.predict(net, pred_loader))
  data4d = np.moveaxis(pred.cpu().detach().numpy().reshape((t.shape[0],nx,ny,nz)),0,3)
  nib.save(nib.Nifti1Image(np.float32(data4d),image.affine), 'pred.nii.gz')  
