...
#  PD_patches = None
  ...
  mask_extract = 0.5
  ratio = 6 #we extract more patches than necessary due to mask checking (which leads to unknown number of patches)
  random_state = None
  extraction_step = 1
...
#    k_s = rng.randint(vol4d.shape[2] - patch_shape[2] + 1, size=n_patches)
    ...
    T1_patches[i*n_images:i*n_images+npe,:,:,:] = pmT1[0:npe,:,:,:]
    T2_patches[i*n_images:i*n_images+npe,:,:,:] = pmT2[0:npe,:,:,:]
    PD_patches[i*n_images:i*n_images+npe,:,:,:] = pmPD[0:npe,:,:,:]
    
  #Remove zeros if necessary (meaning that less than n_patches have been found for some images)
  #Checking only the max value 
#  index = (np.max(T1_patches.reshape(T1_patches.shape[0],-1),axis=1)==0)
